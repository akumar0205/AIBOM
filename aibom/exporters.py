...
def export_spdx(aibom: dict[str, Any]) -> dict[str, Any]:
    ...
    for model in aibom.get("models", []):
        packages.append(
            {
                "SPDXID": f"SPDXRef-Model-{model['type']}",
                "name": model["type"],
                "versionInfo": model.get("model", "unknown"),
            }
        )
    for tool in aibom.get("tools", []):
        packages.append(
            {
                "SPDXID": f"SPDXRef-Tool-{tool['name']}",
                "name": tool["name"],
                "versionInfo": "unknown",
            }
        )
    for ds in aibom.get("datasets", []):
        packages.append(
            {
                "SPDXID": f"SPDXRef-Dataset-{ds['type'].replace('.', '-')}",
                "name": ds["type"],
                "versionInfo": "unknown",
            }
        )
        ...
        components.append(
            {
                ...
                "name": model["type"],
                "version": model.get("model", "unknown"),
            }
        )
